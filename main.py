...
def read_data():
    tot_image_data = []
    for idx, filename in enumerate(os.listdir('D:/source/data')):
        im = Image.open('D:/source/data/' + str(filename))
        load_img = im.load()
        temp_data = []
        for i in range(0, 64):
            for j in range(0, 64):
...
def create_image(images, epoch):
    for idx in range(0, len(images)):
        ...
        if not os.path.isdir('d:/gen_image\\3th_test\\' + str(epoch)):
            os.mkdir('d:/gen_image\\3th_test\\' + str(epoch))

        new_img.save('d:/gen_image\\3th_test\\' + str(epoch) + '\\' + str(idx) + '.jpeg')

def save_image(images, sess, epoch):
    generated = sess.run(images)

    with open('d:/gen_image\\3th_test\\' + str(epoch) + '.jpeg', 'wb') as f:
        f.write(generated)
